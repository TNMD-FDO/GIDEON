...
**Status:** 0.x — slice 0 (platform) complete at `v0.1.0`, the clean-VM acceptance at minor tags its standing proof; slice 1 (General) complete at `v0.2.0`, with no user on the box until go-live (ADR-0044); slice 2 (eval harness) under way toward `v0.3.0`; this tree is `v0.2.41`.
...
| Trigger registry (the committed improvement decisions and conditions) | [`config/triggers.yaml`](config/triggers.yaml) · [`docs/archi/improvement.md`](docs/archi/improvement.md) |
...
| The proposals report (`python3 -m gideon proposals` on the box, from the release checkout: every proposal waiting on a person, read-only — each `watching` trigger in `config/triggers.yaml` fired, not fired, or not yet measurable with the figures that decided it; product sections on the build box alone, office sections on every box; it writes nothing, and exits 1 only when a section could not be read) | [`gideon/improvement/`](gideon/improvement/) · [`config/triggers.yaml`](config/triggers.yaml) · [ADR-0049](docs/adr/0049-improvement-loops-propose-only-a-persons-explicit-act-changes-what-gideon-answers.md) |
