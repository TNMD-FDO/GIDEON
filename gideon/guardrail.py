...
The code enforcement of the no-model-arithmetic rule: pattern families judged in
``FAMILIES`` order — the deadline family, the Guidelines family, sentence
credit — each carrying its refusal, its figure form and normaliser (the figures
a user may supply, keyed canonically), its restatement constructions, its
confirmation context, and its own pattern-set version; the judge's entries over
a text, a rendered message, and a floor; the lag window's state and check over
