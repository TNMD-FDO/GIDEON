...
| [`archi/workflow-tools.md`](archi/workflow-tools.md) | the development repository's tooling, which the export omits: the cycles record, the tracker board, the cycle launcher, the release's git tail, the worktree scripts, the public export |
...
| `python3 -m tools.cycles` · `python3 -m tools.tracker` · `bin/trip` · `bin/release-git` · `bin/worktree-claim` · `bin/worktree-remove` · `bin/release-export`, each its own module or script (development repository) | [`archi/workflow-tools.md`](archi/workflow-tools.md) |
