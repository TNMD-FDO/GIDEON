...
- **Key material never enters Python**: every key and password reaches its consumer as a path (`*_FILE`), a Compose secret mount, a 0600 env file, a file read inside the consumer's own container, or stdin; a generated secret is minted absent-only into a 0440 file, or rotated in place by `secrets rotate`; none appears in argv, logs, refusals, or reprs. The mechanism per secret is [`archi/render-apply.md`](archi/render-apply.md)'s (secrets, the rendered tree), [`archi/engine-frontend.md`](archi/engine-frontend.md)'s (`engine verify`), and [`archi/tools.md`](archi/tools.md)'s (the turn harness, whose `--out` records are never committed and whose browser test account the product never knows); an on-box proof's is `docs/agents/on-box-proofs.md`'s (development repository).
