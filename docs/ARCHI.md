...
This document records **how the code is built** — structure, conventions, commands — for anyone (human or agent) about to implement in this repo, as the code exists and not as the destination. For *what* is being built and *why*, the authorities are, in precedence order: `.scratch/greenfield-spec/spec.md` (the build spec), the ticket Answers under `.scratch/greenfield-spec/issues/`, [`CONTEXT.md`](../CONTEXT.md) (the vocabulary), and [`docs/adr/`](adr/) (the decision records). `§n` references point into the spec; this document never restates a spec decision it can cite.
...
| [`archi/tools.md`](archi/tools.md) | the pin watch and the research notes' pin bindings, the build tool, the acceptance harness, the turn harness and its browser mode, the gate, the cycles record, the tracker board, the release's git tail; their interpreters and footprints | `tools.pinwatch` and its `.notes`, `.hub`, `.bumped`; `tools.imagebuild`, `tools.acceptance`, `tools.turns`, `tools.cycles`, `tools.tracker` |
...
│   └── gate.py · cycles.py · tracker.py · exportboundary.py   # the gate, the cycles record, the tracker board, the export boundary's list — the dev venv, never the box
├── bin/                     # trip, the cycle launcher; release-git and worktree-claim, the guard's git as one plain command each; release-export, the public export (archi/tools.md); excluded from the export
...
- **The acceptance run is at minor tags** (ADR-0035): a minor or major release runs `tools.acceptance` against the local tag between the tag and the merge (the `acceptance-run` block), an evidence commit carrying the transcripts; the pushed tag runs it again on the box's runner as the record; patch tags never run it — the box's own upgrade proves them.
...
| 1 General | `v0.2.0` | **In progress** — landed through `v0.1.40`: the leaves' slice-1 rows. What remains is the board's: the pre-launch release on the critical path to the tag, the turn harness's decline form, the later guardrail families | [`archi/host.md`](archi/host.md) (the profile and its memory table), [`archi/render-apply.md`](archi/render-apply.md), [`archi/engine-frontend.md`](archi/engine-frontend.md), [`archi/stack.md`](archi/stack.md), [`archi/tools.md`](archi/tools.md), [`archi/tests.md`](archi/tests.md) |
...
- **Key material never enters Python**: every key and password reaches its consumer as a path (`*_FILE`), a Compose secret mount, a 0600 env file, a file read inside the consumer's own container, or stdin; a generated secret is minted absent-only into a 0440 file, or rotated in place by `secrets rotate`; none appears in argv, logs, refusals, or reprs. The mechanism per secret is [`archi/render-apply.md`](archi/render-apply.md)'s (secrets, the rendered tree), [`archi/engine-frontend.md`](archi/engine-frontend.md)'s (`engine verify`), and [`archi/tools.md`](archi/tools.md)'s (the turn harness, whose `--out` records — the model's text, for the session — are never committed, and whose browser mode's test account the product never knows exists). An on-box proof passes a secret only by file or stdin, because sudo journals an expanded command line ([`agents/on-box-proofs.md`](agents/on-box-proofs.md)'s secret rule).
...
- **Evidence**: no secret and no office value in the tree — the acceptance harness and the on-box command `python3 -m tools.redact` redact at capture, the print-once secrets and every value of the site registry's marked leaves alike ([`archi/tools.md`](archi/tools.md)); `tests/test_evidence_hygiene.py` holds `.scratch/` to the secrets and `tests/test_office_values.py` the whole tree to documentation values and site-key placeholders, naming no office value itself — public as a **filtered export** of each tag, `bin/release-export`'s ([`archi/tools.md`](archi/tools.md)): the paths that stay here are `tools/exportboundary.py`'s one list, mirrored in `.gitattributes` and held by `tests/test_export_boundary.py`, which also holds every kept document and product string to it; a test that reads an excluded path skips in an export tree by `in_export_tree`, never by the absence of `.git` (slice-1 tickets 56 and 57).
