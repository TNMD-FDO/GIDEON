...
│       ├── render/          # the §3.5 pure render core: the ARTIFACTS registry, one module per artifact family, consumers, command, the drill's and `gideon-ci`'s documents (`drill.py`, `ci.py`)
...
│   ├── pinwatch/ · imagebuild/ · acceptance/ · turns/ · redact/ · cistack/ · courtmap/   # the pin watch (hosted CI); the build tool, the clean-VM harness, the turn harness's entry, the evidence redaction, the `gideon-ci` sibling stack and its engine relay (the box); the court map generator (by hand)
...
| `frontend-contract` (`ci.yml`) | after `mirror-images`, same runner | `python3 -m unittest tests/contract/owui_apply_manifest.py`: a throwaway Postgres + frontend pair on loopback, the sentinel's stub its engine, proves the apply manifest is desired state ([`archi/tests.md`](archi/tests.md)) |
...
| 2 Eval harness | `v0.3.0` | **In progress** — the `extraction` slice frozen and measured, the judge grading on its own, the harvest's questions committed as `research-qa` cases in the unsigned state, stdlib alone (ADR-0046), the turn harness under `eval`, and the standing `gideon-ci` sibling stack [19]; what remains: the suites [12]–[15], decision and nightly runs and their board [16]–[18], the push-time smoke on the sibling [20], the graders [21] | `eval`, `eval-slices` |
