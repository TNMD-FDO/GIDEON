...
| [`archi/eval.md`](archi/eval.md) | the eval harness: the exact-object contract, the extraction grammar and its measure, the judge, the set's cases, slices and references, the two `eval` commands and the record |
...
│   ├── evaluation/          # the eval-set loader, the slice registry and its runners, the judge, the quiet window, the recorder, the reference format, the two `eval` commands (archi/eval.md)
...
│   └── gate.py · cycles.py · tracker.py · judgments/ · variants/ · exportboundary.py   # the gate, the cycles record, tracker board, the judgment and variant generators, the export list — the dev venv
...
├── eval/                    # reference/<slice>/<list>.json (the gate's committed comparand, §18.5); seed/prototype-qa/ (frozen harvest tooling, ruff-excluded, do not modify); sets/eval-v1/ (the set's cases by suite — judgments/, build-gates/, judge/ — and its frozen slices, slices/<name>/<file>.ids; archi/eval.md);
...
| `eval run --slice <name> [--set <dir>]` (`evaluation/command.py`) · `eval reference --run <id>` (`evaluation/reference_command.py`) · `python3 -m tools.variants` | [`archi/eval.md`](archi/eval.md) |
...
| 2 Eval harness | `v0.3.0` | **In progress** — the `extraction` slice frozen and measured and the judge grading on its own, stdlib alone (ADR-0046); what remains: the eval metrics [09]–[10], the turn harness and the suites [11]–[15], decision and nightly runs and their board [16]–[18], the sibling stack [19]–[20], the graders [21] | `eval` |
...
- **Least privilege**: the audit writer role holds INSERT and a partition function on its two tables only, the frontend its second consumer for the guardrail's trip writer; `gideon_eval` holds the same on its own two tables, insert-only, the metrics role its reader ([`archi/eval.md`](archi/eval.md)); the frontend's endpoint restrictions bind the admin key too, so the release's allowlist is exactly what apply, reconcile, and the eval identity call; the push sends no ownership to the plain NAS account; the pin watch's App token is minted per run, hour-scoped, and never in a row; the CI runner's one sudoers line names one module of the checkout it runs from; the tag-triggered run's trust is who may create a `v*` tag — the collaborator list where the run happens (slice-1 ticket 57, ADR-0031).
...
- **Evidence**: no secret or office value in the tree — the acceptance harness and on-box `python3 -m tools.redact` redact at capture, including print-once secrets and marked site leaves ([`archi/tools.md`](archi/tools.md)); `tests/test_evidence_hygiene.py` holds `.scratch/` to secrets and `tests/test_office_values.py` the tree to documentation values and site-key placeholders, naming none. The public repository, a **filtered export** of each tag by `bin/release-export`, carries the application alone: `tools/exportboundary.py`'s list of development files is held by `tests/test_export_boundary.py`, and a test reading an excluded path skips there by that module's predicates, never by the absence of `.git` ([`archi/tests.md`](archi/tests.md); slice-1 tickets 56, 57, and 79).
