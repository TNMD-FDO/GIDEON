...
| 2 Eval harness | `v0.3.0` | **In progress** — the `extraction` slice frozen and measured and the judge grading on its own, stdlib alone (ADR-0046); what remains: the eval metrics [10], the turn harness and the suites [11]–[15], decision and nightly runs and their board [16]–[18], the sibling stack [19]–[20], the graders [21] | `eval`, `eval-slices` |
