...
| [`archi/workflow-tools.md`](archi/workflow-tools.md) | the development repository's tooling, which the export omits: the cycles record, the tracker board, the cycle launcher, the release's git tail, the worktree scripts, the public export, the judgment set's intake |
| [`archi/eval.md`](archi/eval.md) | the eval harness: the exact-object contract, the extraction grammar and its measure, the eval set's cases and frozen slices, `eval run` and its record |
...
│   ├── evaluation/          # the eval-set loader, the slice runners, the run recorder, `eval run`'s ordered command (archi/eval.md)
...
├── bin/                     # the development repository's scripts — the cycle launcher, the guard's git as one plain command each, the public export (archi/workflow-tools.md); excluded from the export
...
├── eval/                    # reference/ (empty skeleton, §18.5); seed/prototype-qa/ (frozen harvest tooling, ruff-excluded, do not modify); sets/eval-v1/ (the eval set's cases by suite — judgments/, build-gates/ — and its frozen slices, slices/<name>/<file>.ids, each README the authority; archi/eval.md);
...
The tools live in an untracked project venv — `python3 -m venv .venv && .venv/bin/pip install -r requirements-dev.txt`; CI's `checks` job installs from the same file, whose two copies §3's test holds to it.
...
| `eval run --slice <name> [--set <dir>]` (`evaluation/command.py`) | [`archi/eval.md`](archi/eval.md) |
...
| `python3 -m tools.cycles` · `python3 -m tools.tracker` · `bin/trip` · `bin/release-git` · `bin/worktree-claim` · `bin/worktree-remove` · `bin/release-export` · `python3 -m tools.judgments.intake`, each its own module or script (development repository) | [`archi/workflow-tools.md`](archi/workflow-tools.md) |
...
- **The version lives in exactly one place**: `__version__` in [`gideon/__init__.py`](../gideon/__init__.py). `pyproject.toml` reads it via `[tool.setuptools.dynamic]`; `--version` prints it. The release writes it there alone, after the rebase, and `bin/release-git` derives every other copy ([`archi/workflow-tools.md`](archi/workflow-tools.md)), the README's status tag held equal to it by `tests/test_release_records.py` (slice-1 ticket 60, workflow ticket 38).
- **Two release documents** (§2.1, §21): the changelog `docs/2-changelog/w<N>_v<x.y.z>.md` is the engineering record and the one `upgrade` reads for its `## Breaking` section; the release note `docs/release-notes/v<x.y.z>.md`, written from the versioned `TEMPLATE.md`, is the user-facing artifact the CSAs send, its bumped-pins section generated ([`archi/tools.md`](archi/tools.md)). The root `CHANGELOG.md` indexes every changelog file newest first (spec §2.2); a hotfix tag has a changelog file like any tag. `tests/test_release_notes.py` and `tests/test_release_records.py` hold the three ([`archi/tests.md`](archi/tests.md)), and the release skill's `user-facing-tag-check` and `changelog-index` blocks write them.
...
| 1 General | `v0.2.0` | **Complete** at `v0.2.0`, the pre-launch release, with no user on the box until go-live (ADR-0044) — the leaves' slice-1 rows are the record; its open follow-ons are the board's | `host` (the profile and its memory table), `render-apply`, `engine-frontend`, `stack`, `tools`, `workflow-tools`, `tests` |
| 2 Eval harness | `v0.3.0` | **In progress** — `eval run` records the `extraction` slice, stdlib alone (ADR-0046); what remains: the grammar's families [02], the reference run [05], the judge and grading kit [06], [08]–[10], the turn harness and the suites [11]–[15], decision and nightly runs and their board [16]–[18], the sibling stack [19]–[20], the graders [21] | `eval` |
...
flowchart LR
    ...
    E -->|"eval run --slice <name>"| ER["evaluation/command.py: load → run → record → gate<br/>(archi/eval.md)"]
    ER --> ST
...
- **Key material never enters Python**: every key and password reaches its consumer as a path (`*_FILE`), a Compose secret mount, a 0600 env file, a file read inside the consumer's own container, or stdin; a generated secret is minted absent-only into a 0440 file, or rotated in place by `secrets rotate`; none appears in argv, logs, refusals, or reprs. `gideon-api` reads the generated `gideon_api_key` and the engine key from Compose-mounted files, while the engine key has the service as its second mount consumer. The mechanism per secret is [`archi/render-apply.md`](archi/render-apply.md)'s (secrets, the rendered tree), [`archi/engine-frontend.md`](archi/engine-frontend.md)'s (`engine verify`), and [`archi/tools.md`](archi/tools.md)'s (the turn harness, whose `--out` records are never committed and whose browser test account the product never knows); an on-box proof's is `docs/agents/on-box-proofs.md`'s (development repository).
...
An image bump reaches the running stack at the next `apply`, which recreates the bumped service by its Compose block (the recreate rule names it; Compose's own config diff is the backstop the row does not model). Off the box, every Monday a CSA reviews the open `pin-watch/*` pull requests and Dependabot's beside them (`docs/runbooks/pin-watch-app-setup.md` §5, a frontend bump's on-box proof before its merge there too); a built-pin proposal is completed by a rebuild on the box per `docs/runbooks/built-images.md` (build → record → commit → merge, never the reverse).
