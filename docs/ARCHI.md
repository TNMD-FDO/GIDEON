...
- **The acceptance run is at minor tags** (ADR-0035): a minor or major release runs `tools.acceptance` against the local tag between the tag and the merge (the `acceptance-run` block), then its clean-VM full restore of the box's newest set (`--full-restore`, detached; §2.5 item 3, ADR-0042), an evidence commit carrying both runs' transcripts; the pushed tag runs it again on the box's runner as the record; patch tags never run it — the box's own upgrade proves them.
