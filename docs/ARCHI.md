...
| [`archi/eval.md`](archi/eval.md) | the eval harness: the exact-object contract, the extraction grammar and its measure, the eval set's cases |
...
├── eval/                    # reference/ (empty skeleton, §18.5); seed/prototype-qa/ (frozen harvest tooling, ruff-excluded, do not modify); sets/eval-v1/ (the eval set's cases by suite — judgments/, build-gates/ — each README the authority; archi/eval.md);
...
| 2 Eval harness | `v0.3.0` | **In progress** — `gideon eval` (plain Python: stdlib + httpx + numpy, §18.1), the extraction grammar's remaining families and variants | `eval` |
