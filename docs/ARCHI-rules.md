...
The plan's Documentation Impact names the leaves. Plan reads the map, then the leaves the ticket's commands, modules, and tests belong to; implement reads the plan and those leaves, `python3 -m tools.archi where` its fallback when the code disagrees with a leaf, which answers for a command what the map's §8 table holds; research reads the map, then the leaves its question needs. More than three leaves is a ticket to split. A cross-cutting leaf is not counted toward the three: a plan reads one only when its ticket adds or changes a mechanism of a kind its section's stub names, and Documentation Impact names it as it names any leaf. Release Step 7 fires when the release's diff adds, removes, or renames a command, a module, a release constant, or a test module, and then updates the named leaves and the map wherever the map's own content changed: §2 and §12 only when a subsystem appears or a slice moves, §8's table when a command lands or moves, and any cross-cutting section whose fact moved.
...
Each leaf has, in order: a front-matter block with `commands` and `modules` lists and optional `indexed` (the longest matching module entry owns a path; `tests.md` and every cross-cutting leaf use both lists empty); an H1 `# <Title> — architecture leaf`; a **Read this when** paragraph naming what the leaf is read for, ending with a link to the map and the note that the map's §8 table names every command's leaf; `## Inventory`, the three-column table (surface, code, spec/ADR) the map's §2 once held, one line per surface; `## Commands`, one bullet per command with its stages and invariants; the leaf-specific parts (`## Release constants by module`, `## Configuration`, `## The locks`, `## Footprints`, or a one-line citation of the leaf that owns them); `## Tests`, its modules one line each, a module declared by a bullet opening with its backticked module names, which the check reads; and `## Cross-references` with a *Cites* bullet (the leaves it depends on, as relative links) and a *Cited by* bullet (the map sections that point at it). Relative links resolve from `docs/archi/`; a file that does not exist yet is named in code font, never linked.

A cross-cutting leaf has, in order: a front-matter block with both lists empty; an H1 `# <Concern> — cross-cutting leaf`; a **Read this when** paragraph naming the map section whose stub it serves and the kinds of mechanism it holds, saying that the section keeps the rules that bind all code, and ending with the link to the map; no Inventory and no Commands part, since it owns no surface and no command; one `## <Kind>` part per kind of mechanism the section held, the section's lines moved whole, a mechanism whose code a subsystem leaf owns pointing there; and `## Cross-references` with the same two bullets, the *Cited by* naming the section. A tripwire that holds a posture stays [`tests.md`](archi/tests.md)'s.
...
The map, a leaf, `CLAUDE.md`, or a document under `docs/agents/` past its warning is compacted with `TRIP-compact <file>` in a tracker session on `main`, as one `tracker:` commit whose body is the loss ledger, never inside a release: the release that sees the crossing names the file among its Step 15 leftovers. A compaction keeps each leaf's block, and a split writes one for each half. The skill is user-invoked with the file as its argument, which overrides its own target line; its own ~20k thresholds are ignored (its "within acceptable range" question is answered yes), the count that matters being the count script's on that file. The ledger is built from a diff of the committed text against the compacted text — the backticked tokens and named rules present in the old file and absent from the new: for every section cut by more than about a third, what left and where it lives now (a module, a test, a ticket, another document), true losses separated from text that moved or is duplicated elsewhere; a pin value or measurement the document restated against its own rules is a correction, not a loss. If a leaf cannot come under its warning, split it by mechanism — [`render-apply.md`](archi/render-apply.md) split by what grew into [`render-apply.md`](archi/render-apply.md) (the host tree) and [`engine-frontend.md`](archi/engine-frontend.md) (the engine and the frontend's runtime), and [`tools.md`](archi/tools.md) into [`tools.md`](archi/tools.md) (the tools the export carries) and `workflow-tools.md` (the scripts and records the export omits) — naming the seam at the crossing by what grew, since a seam named in advance is drawn against the leaf as it stood then. If the map cannot — a pass finding only content the rules keep — the cross-cutting section that has outgrown it is split: its per-mechanism content moves whole to a cross-cutting leaf under `docs/archi/`, with a move ledger as a leaf's split has, and the section keeps its number, its heading, the rules that bind all code, and the pointer, so every outside citation stands. A sentence stays in the stub when code that never opens the leaf could break it, and moves when only a change to the mechanism it names could: a posture's rule stays as a clause, its mechanism moves. The seam is named at the crossing by what grew among the sections a split can move — a row per leaf or command in §2, §4's tree, §8's table, or §13's diagram is growth no split moves — and never in advance; the split is a move, made without `TRIP-compact` in the same kind of session and commit. §16's split is the standing example: the mechanisms behind its eight postures are [`security.md`](archi/security.md)'s, their rules a clause each in the stub. A kept leaf is indexed from §2, a cross-cutting leaf's row naming its section and what it holds, and one that owns commands is named in §8's table (`tests.md` and a cross-cutting leaf own none and have no row); a leaf the export omits has neither a §2 row nor a §8 row and is routed by the glossary's `_Leaf_` links, `tests/test_archi_budget.py` holding the §2 half of both. If `CLAUDE.md` cannot, move a section's detail whole to a document under `docs/agents/`, keeping the rule and a pointer (workflow ticket 22's record). If an agent document cannot, split it by mechanism as a leaf is, the sentence that pointed at it naming both halves; `on-box-proofs.md`'s named split — §§1, 4, 5 the harness's shapes, §§2, 3, 6–11 the box's — is the standing example.

## Step 7 checklist

The step fires when the release's diff adds, removes, or renames a command, a module, a release constant, or a test module; otherwise the release says so in one line and runs the budget test alone. When it fires: read the leaves named by the plan and any the diff touched. Check the map's §2 and §12 conditions, its §8 table and the leaf's front matter for a landed command, and its cross-cutting sections for a moved fact; a link added to or dropped from the map moves that leaf's Cited-by line in the same change, and the check holds it; `tools/archi.py` and `tests/test_archi.py` hold §8's table, the Cited-by lines, §4's tree, and the ownership of `tools/` and `bin/`. The documents a release touches, listed once here, each the home of one kind of fact:
...
- a cross-cutting leaf — the mechanism behind a posture its section's stub names, the stub changing only when a rule that binds all code does;
