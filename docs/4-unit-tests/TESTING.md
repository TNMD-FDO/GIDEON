...
- A fake `Host` raises `FileNotFoundError` for a missing file, as `RealHost` does — production code never catches a `KeyError` to suit a fake. Its `exists` answers from its dict alone, never falling through to the real filesystem: the build box's own marker (`/etc/gideon/build-box`, declared at `v0.1.71`) turned nine host-mode tests red on the box (`v0.1.72`).
