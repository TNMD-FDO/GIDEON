...
## 6. The CI runner (GitHub Actions)

The box runs the organization's one self-hosted runner (§1.8: user `gh-runner`,
`/opt/gh-runner`, a systemd service, labels `self-hosted,linux,x64,gpu,dl385-gen11`,
registered at `https://github.com/TNMD-FDO`). From `v0.0.20` the `gh-runner`
provision step keeps three facts true and reports drift on any of them: the
runner is registered **with automatic updates disabled** (its settings files
carry `disableUpdate: true` — ADR-0031, nothing on the box checks for its own
updates), the **installed release is the one `host.lock` pins**, and the
service is enabled and active. Everything below is
`sudo python3 -m gideon host provision --only gh-runner`; the facts behind it are
cited in [`docs/research/gh-runner-disableupdate.md`](../research/gh-runner-disableupdate.md).

**The registration token.** Any registration — the first on a fresh box, the
one-time re-registration below, a rebuilt box — needs a registration token:
organization Settings → Actions → Runners → **New runner** → **New self-hosted
runner**, the value after `--token` in the `./config.sh` line the page shows;
or, with admin access to the organization,
`gh api --method POST /orgs/TNMD-FDO/actions/runners/registration-token --jq .token`.
It **expires after one hour** (GitHub's rule), so place it and run provision in
the same sitting. The file is root-owned, mode 0440 (provision's `secrets-dirs`
step created the directory); write it without the token touching a command
line or the shell history — run the first command, paste the token, press
Enter, then Ctrl-D:

    sudo tee /etc/gideon/secrets/gh_runner_token >/dev/null
    sudo chmod 0440 /etc/gideon/secrets/gh_runner_token
    sudo python3 -m gideon host provision --only gh-runner

The step hands the token to `config.sh` through the child's environment (never
argv; the runner masks it in its own trace) and **deletes the file once the
registration succeeds**. A token left on disk is therefore one that was never
used: a refused registration fails the row naming a fresh one, and the next run
picks it up. The refusal's detail is GitHub's own answer; a `404 (Not Found)`
from `POST https://api.github.com/actions/runner-registration` means the token
is not valid for the organization URL — expired, already used, or minted on
the **repository's** runner page (`TNMD-FDO/GIDEON-dev` → Settings) instead of the
**organization's** (`github.com/organizations/TNMD-FDO/settings/actions/runners/new`),
which is the one the box registers with. Without a token the step reports `blocked`
naming the file — an expected mid-runbook state, exit 0.

**The one-time re-registration (`v0.0.20`).** A runner registered before this
release has updates on. The step reports `blocked` naming the token until one is
placed, `blocked` naming the running job while a CI job is in flight (it never
stops a busy runner), then — token present, runner idle — re-registers in one
run: `svc.sh stop` and `uninstall`, `config.sh remove --local` (drops the local
registration, contacts nothing), `config.sh --unattended --replace
--disableupdate …` (takes over the organization's existing `gideon` record,
same runner id, same labels, and sets the record's own updates-off attribute),
`svc.sh install`, the unit enabled, `svc.sh start`. The row reads `applied` and
the announcement names the upgrade path; a second run reads `ok`. The
organization's runner list shows `gideon` idle.

**What pauses.** The runner service is down between the stop and the start —
under a minute. A push to `main` in that window queues `mirror-images` (and the
contract job after it) until the runner returns; a hosted `checks` run is
unaffected; nothing else on the box is touched. If the registration is refused,
the runner stays unregistered with its service uninstalled until a fresh token
and another run — the step's row says so.

**The upgrade path.** With updates off, the runner moves only through a release.
The pin watch opens a `host.gh_runner` pull request within a week of a runner
release (Monday runs; [`pin-watch-app-setup.md`](pin-watch-app-setup.md) §5).
Merge it, then on the box **bring the checkout that runs provision to the
merged commit** — provision reads `host.lock` from the checkout it runs in, so
the merge alone moves nothing on the box (`git pull --ff-only` on `main` in
`~/src/GIDEON` today; `./upgrade.sh <tag>` once ticket 08 lands) — and run
`sudo python3 -m gideon host provision`. The step refuses while a job runs,
stops the service, extracts the pinned release over the install (settings,
credentials, and the work directory survive), refreshes the service wrapper,
removes the superseded archive, and starts the service; a second run reads
`ok`, and the organization's runner list shows the new version.

**The deadline.** GitHub's self-hosted runners reference
(https://docs.github.com/en/actions/reference/runners/self-hosted-runners,
"Runner software updates on self-hosted runners"): with automatic updates
disabled "you will be required to update your runner version within 30 days of
a new version being made available"; "if you do not perform a software update
within 30 days, the GitHub Actions service will not queue jobs to your runner";
and "if a critical security update is required, the GitHub Actions service will
not queue jobs to your runner until it has been updated" — no grace period for
those. GitHub's changelog of 2026-06-12
(https://github.blog/changelog/2026-06-12-github-actions-minimum-version-enforcement-timeline-for-self-hosted-runners/)
announced dated enforcement of that rule for github.com organizations —
brownouts through September, full enforcement on **2026-09-25** — so the
Monday review's merge and the provision run must land inside the month after
each runner release. Both figures are GitHub's, not GIDEON's (`exempt:
GitHub's documented rule`).

**A rebuilt box** (§1.9's wipe and reinstall) registers with `--replace`, so
the organization's old `gideon` record is taken over rather than duplicated;
no cleanup in the runner list is needed. (GitHub removes a runner that has not
connected for 14 days on its own.)

**Checking.** `sudo python3 -m gideon host provision --only gh-runner --dry-run`
prints the step's verdict without touching anything. After a re-registration,
the newest `Runner_*.log` under `/opt/gh-runner/_diag/` reads
`Flag 'disableupdate': 'True'`, and its broker polls carry `disableUpdate=true`;
`/opt/gh-runner/.runner` carries `"disableUpdate": true`.

**Acceptance harness privilege.** The `gh-runner` step also converges this one
sudoers rule:

    gh-runner ALL=(root) NOPASSWD: /usr/bin/python3 -m tools.acceptance *

It writes `/etc/sudoers.d/gideon-acceptance.candidate`, validates the candidate
with `visudo -c -f`, and only then moves it into `/etc/sudoers.d/gideon-acceptance`
at mode 0440, owned by `root:root`. The runner is already root-equivalent through
the docker group (bootstrap ticket 01), never runs pull-request code, and this
line names one module of the checkout it runs from; hardening waits for the
public flip (ADR-0031).

**Tag protection (a repository setting, not code).** `acceptance.yml` runs at
every pushed `v*.*.0` tag as root through that line, and a tag can name any
commit its pusher can reach — a check inside the tag's own workflow cannot
defend it. The control is a GitHub ruleset restricting who may create tags
matching `v*` to the CSAs (Settings → Rules → Rulesets → new tag ruleset,
"Restrict creations"), the same people who may push `main`, which the
`mirror-images` job already trusts with Docker access. GitHub offers rulesets
on a private repository only under Pro or Team, and on the organization's Free
plan only to a public repository. So the ruleset lives on the public
`TNMD-FDO/GIDEON` from slice-1 ticket 57's release — creation, update, and
deletion of `v*` restricted, the Repository admin role bypassing, proven there
by a refused tag push — and guards the tags an office clones. The development
repository `TNMD-FDO/GIDEON-dev`, where `acceptance.yml` runs, is private on
the Free plan and has none: its control is the collaborator list itself
(`.scratch/slice-0/issues/22-public-repository.md`, ruled 2026-09-07), two
accounts, both admin, both CSAs, confirmed at the Monday review
([`pin-watch-app-setup.md`](pin-watch-app-setup.md) §5). A third account with
write access is the trigger to revisit that ticket.
