...
- **Merge** it when its hosted checks are green and the bump is wanted (a frontend bump: below). A merge to `main` runs the box's `mirror-images` and `frontend-contract` jobs (for an image pin) — the §17.5 contract gate — and the running stack moves only at the next `gideon apply`. A human then tags the release (§2.1); the tag's release note names what was bumped.
...
- A **frontend bump** (`images.open-webui`, plain or proposal) is proven on the box before it merges. In a window — free before go-live, a maintenance window from it (§21) — announced to Transcribe's operator (the box ledger's §8), the open tickets and `pin-watch/images.*` and `host.*` pull requests read for riders: the bump's branch checked out on the box, never `/opt/gideon` or the primary, current with `main` (a dispatched watch run); `backup run`, then `registry mirror` and `apply` from that checkout, `render --diff`'s row naming the frontend alone; the turn harness's API mode on `eval/seed/general/frontend-bump.yaml` and on the smoke set, `search-01` reading `sources present`; its browser mode with `--probe-inlet` on the cases file (`--force` said so outside a window), the engine's chat-completion lines per uncancelled browser turn other than 3 a ticket (the cancelled case and the probe counted apart); the notes the body lists read against the new tag, amended as a `docs:` commit on `main` (a digest-only bump stands on the green run); both modes' rows on the pull request, ids, classes, and seconds only. A selector or route that no longer holds is fixed under its own plan on its own branch, merged to `main` first, never merged broken; a red bump is undone by an `apply` from `/opt/gideon` (a tag move that migrated the database by `restore`, ADR-0005), and the checkout stays until the next tag's `upgrade` from `/opt/gideon`.
