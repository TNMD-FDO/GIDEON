...
| node exporter (host, systemd units), DCGM exporter (GPUs), Postgres exporter, cAdvisor (containers), blackbox exporter (the served certificate, the frontend's health, SearXNG's health while `web.search` is on, and `gideon-api`'s `/health` on a GPU host); the engine's own `/metrics` (KV-cache usage, queue depth — a GPU host only, keyless by vLLM's design) | Compose services on the private network | not published anywhere |
...
| API probe failing | for 10 minutes: `gideon-api`'s health probe fails on a GPU host | `docker compose -f /etc/gideon/rendered/compose.yaml ps gideon-api` and `logs gideon-api`; `sudo python3 -m gideon apply` brings the service and its mounted checkout back to desired state |
...
backup keypair keeps its own ownership). Grafana, the Postgres exporter, and
`gideon-api` run unprivileged and read their secrets through that group
(`group_add` in the rendered Compose file); `gideon-api` reads both the engine
and API key files. A secret a CSA places by hand as `root:root` converges
