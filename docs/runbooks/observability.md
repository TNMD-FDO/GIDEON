...
| backup unit failed | `gideon-backup`, `gideon-backup-drill`, or `gideon-backup-verify` is in systemd's `failed` state — a failed push or off-box check lands here too, since a failed push writes no row | the unit's journal; fix the cause; `systemctl reset-failed <unit>` after a hand-run succeeds |
